...
- **Google Fonts are loaded from Google's CDN**, not self-hosted (unlike the site's other three font families, which live in `public/fonts/`). The CSP in `src/middleware.ts` explicitly allows `fonts.googleapis.com`/`fonts.gstatic.com` for this reason. Self-hosting them would remove one third-party network dependency from the critical rendering path -- a reasonable follow-up, not currently blocking.
