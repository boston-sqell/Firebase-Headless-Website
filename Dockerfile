# Pinned by digest (not just the floating "22-slim" tag) so the exact same
...
# means here. To refresh when a new Node 22 patch/security release ships:
...
#     https://registry-1.docker.io/v2/library/node/manifests/22-slim -D - -o /dev/null | grep -i docker-content-digest
